...
from human_aware_rl.human.data_processing_utils import json_joint_action_to_python_action
...
def is_insertion_needed(state):
    ...
    if "objects" not in state:
        raise ValueError("Corrupted data detected, state missing 'objects' key")
    ...
    for soup in soups:
        if not 'cooking_tick' in soup:
            raise ValueError("Legacy schema detected! Please ensure you are using updated state schema")
...
def main(data_infile, data_outdir, insert_interacts, forward_port, fix_json, verbose):
    """
    Arguments:
    ...
    for i, datapoint in raw_data.iterrows():
        ...
        state_dict, insertion_needed = process_state(state, forward_port, fix_json)
